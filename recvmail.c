...
void delete_recv_mail(GtkWidget *widget, gpointer data)
{
	if (selected_row == -1)
		return;
	recvWindow *p = (recvWindow*)data;
...
void search_recv_mail(GtkWidget *widget, gpointer data)
{
	recvWindow *p = (recvWindow*)data;
	GtkWidget *dialog;
...
void forward_recv_mail(GtkWidget *widget, gpointer data)
{
	recvWindow *p = (recvWindow*)data;
	GtkWidget *dialog;
...
void recv_add_to_trash(GtkWidget *widget, gpointer data)
{
	if (selected_row == -1)
		return;
	recvWindow *p = (recvWindow*)data;
...
void recv_mail(const gchar *account)
{
	...
	g_signal_connect(G_OBJECT(rwindow.outbox_trash_button), "clicked", G_CALLBACK(recv_add_to_trash), (gpointer)&rwindow);
	g_signal_connect(G_OBJECT(rwindow.outbox_show_button),"clicked",G_CALLBACK(show_recv_window), (gpointer)&rwindow);
	g_signal_connect(G_OBJECT(rwindow.outbox_delete_button), "clicked", G_CALLBACK(delete_recv_mail), (gpointer)&rwindow);
	g_signal_connect(G_OBJECT(rwindow.outbox_forward_button), "clicked", G_CALLBACK(forward_recv_mail), (gpointer)&rwindow);
	g_signal_connect(G_OBJECT(rwindow.outbox_search_button), "clicked", G_CALLBACK(search_recv_mail), (gpointer)&rwindow);
